...
//基本图元：圆
    ...
    //圆弧半径
    __DATA__ int radius;
    __DATA__ int angle_start;  //起始角度
    __DATA__ int angle_end;    //终止角度

    __DATA__ float angle; //倾角

    //继承自graph的方法----------------------------------------------------------------------
    //添加子元：返回值：图元序号
    __FUNCTION__ int (*insert)(struct graph_arc *cthis,struct graph_arc *item);
    //删除子元
    __FUNCTION__ int (*del)(struct graph_arc *cthis,int item_id);
    //返回子元序列
    __FUNCTION__ struct graph** (*get_child)(struct graph_arc *cthis);
    //返回父元
    __FUNCTION__ struct graph* (*get_parent)(struct graph_arc *cthis);
    //返回子代数量
    __FUNCTION__ int (*get_child_count)(struct graph_arc *cthis);

    //覆写的函数------------------------------------------------------------------------------
    //构造函数
    __FUNCTION__ __OVERRIDE__ void (*init)(struct graph_arc * cthis,
                                           struct point center,
                                           int radius,
                                           float angle_start,
                                           float angle_end,
                                           float angle);
    //析构函数
    __FUNCTION__ __OVERRIDE__ void (*exit)(struct graph_arc *cthis);
    //绘制图形
    __FUNCTION__ __OVERRIDE__ int (*draw)(struct graph_arc *cthis,struct window * draw_object);
    //判断是否相交
    __FUNCTION__ __OVERRIDE__ int (*is_cross)(struct graph_arc *cthis,struct point point);
    //平移
